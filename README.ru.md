...
> дизайном в стиле **Glassmorphism**, полноценной поддержкой темной и светлой
> тем, синхронизацией состояния с URL, отзывчивой мобильной версией, тактильным
...
Проект спроектирован с упором на строгое разделение ответственности, модульность
и соответствие соглашениям (BEM, SCSS Modules). Все ключевые директории имеют
понятную структуру:

```text
src/
├── app/            # Инициализация Redux Store и конфигурация роутера (React Router v7)
├── components/     # Переиспользуемые UI-компоненты
│   ├── common/     # Универсальные атомарные компоненты (кнопки, скелетоны, заглушки)
│   ├── cart/       # Компоненты корзины (боковое меню, элементы корзины)
│   ├── products/   # Карточки товаров, фильтры, элементы пагинации
│   └── layout/     # Компоненты разметки (Header, Navbar, Footer, обертки страниц)
├── config/         # Настройки проекта и интеграций (Supabase клиент)
├── hooks/          # Кастомные React-хуки (бизнес-логика вынесена из UI-компонентов)
│   ├── common/     # Общие хуки (useTheme, useHaptics, usePagination)
│   └── features/   # Хуки фич (cart, checkout, order, product, wishlist, auth)
├── pages/          # Страницы приложения (CatalogPage, ProductPage, OrdersPage и др.)
├── schemas/        # Схемы валидации Zod для форм (вход, регистрация, редактирование профиля)
├── services/       # Слой данных RTK Query API slices (продукты, корзина, избранное, заказы)
├── store/          # Redux-срезы (slices), селекторы (Reselect) и настройки персистенции
├── styles/         # Дизайн-система (SCSS-переменные, темы, глобальные стили)
├── types/          # Общие типы и интерфейсы TypeScript
└── utils/          # Чистые утилиты и хелперы (форматирование цен, пресеты haptics)
```

### Ключевые архитектурные решения

1. **URL как единственный источник правды (Single Source of Truth) для
...
2. **Тонкие компоненты и логика в хуках**: Все UI-компоненты декларативны.
   Запросы к API, манипуляции с роутером, вычисление производных состояний и
   валидация происходят в специализированных хуках (например, `useCartDetails`,
   `useProductDetails`, `useWishlistDetails`).
3. **Нормализация кэша в RTK Query**: Ответы API от DummyJSON трансформируются в
   структуру `{ ids: number[], items: Record<number, Product> }` для O(1) поиска
   и снижения нагрузки на рендеринг.
4. **Селективное кэширование в LocalStorage (Redux Persist)**: В локальное
   хранилище записываются только самые важные данные: `cart.items`,
   `wishlistSlice` (для неавторизованных пользователей), `theme.theme` и `auth`
   (пользователь и токен). Кэш запросов RTK Query и временное состояние UI не
   персистируются.
5. **Тематизация на основе CSS Custom Properties**: Переключение тем
   (`theme-dark` / `theme-light`) осуществляется добавлением класса на элемент
   `<html>`. Стили компонентов используют единую систему CSS-переменных, что
   исключает лишние ререндеры дерева React при переключении темы.
...
- 🌗 **Умная смена тем**: Полная поддержка темной (глубокий фиолетовый космос) и
  светлой тем с мгновенным автоматическим переключением и сохранением выбора
  пользователя.
...
- 🚀 **Асинхронность и ленивая загрузка**: Все страницы (кроме главной
  ...
  (`react-loading-skeleton`), стилизованные под CSS-переменные активной темы.
...
- [x] **Архитектурный фундамент**: React 19, TS 5.9, Vite 8, LightningCSS, RTK
      Query.
- [x] **Система стилей**: SCSS Modules по BEM, переменные и переключатель тем
      (Light / Dark).
