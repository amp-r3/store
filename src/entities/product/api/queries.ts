...
export async function fetchProducts(
  ...
  let query = db.from('products_view').select('*', { count: 'exact' });
...
export async function fetchCategories(db: SupabaseClient<Database>): Promise<Categories> {
  const { data, error } = await db.from('categories').select('slug, name');
...
export async function fetchProductById(db: SupabaseClient<Database>, id: number): Promise<Product> {
  const { data, error } = await db.from('products_view').select('*').eq('id', id).single();
...
export async function fetchProductArrayById(
  db: SupabaseClient<Database>,
  ids: number[],
): Promise<Product[]> {
  const { data, error } = await db.from('products_view').select('*').in('id', ids);
...
export async function fetchDealsProducts(
  db: SupabaseClient<Database>,
  params?: { limit?: number },
): Promise<Product[]> {
  const limit = params?.limit ?? 12;
