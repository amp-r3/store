...
export const reviewApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getMyReviews: builder.query<ProductReview[], void>({
      async queryFn() {
        try {
          ...
          const reviews = data.map((review) => mapReview(review, new Set<number>()));
    ...
    addOrUpdateReview: builder.mutation<
      void,
      { productId: number; rating: number; comment: string; reviewerName?: string; userId?: string }
    >({
      ...
      async onQueryStarted(
        { productId, rating, comment, reviewerName, userId },
            ...
            dispatch(
              reviewApi.util.updateQueryData('getReviews', originalArgs, (draft) => {
                ...
                const existingIndex = draft.items.findIndex(
                  (review) => review.userId === resolvedUserId,
                );
    ...
    deleteReview: builder.mutation<void, { reviewId: number; productId: number }>({
      ...
      async onQueryStarted({ reviewId, productId }, { dispatch, queryFulfilled, getState }) {
        ...
        const myReviews = reviewApi.endpoints.getMyReviews.select()(getState()).data;
        const deletedRating =
          myReviews?.find((review) => review.id === reviewId)?.rating ??
          invalidatedEntries
            .map(
              ({ originalArgs }) =>
                reviewApi.endpoints.getReviews
                  .select(originalArgs)(getState())
                  .data?.items.find((review) => review.id === reviewId)?.rating,
            )
