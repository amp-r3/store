...
export const CartItem = memo<CartItemProps>(
  ({ product, onIncrease, onDecrease, onRemove, onClose, readonly = false }) => {
    ...
    return (
      <article
        className={`${styles['cart-item']} ${readonly ? styles['cart-item--readonly'] : ''}`.trim()}
      >
        <Link
          href={`/product/${id}`}
        ...
        <div className={styles['cart-item__content']}>
          <div className={styles['cart-item__header']}>
            ...
            <div className={styles['cart-item__price-group']}>
              <span className={styles['cart-item__price']}>{formatPrice(totalPrice)}</span>
