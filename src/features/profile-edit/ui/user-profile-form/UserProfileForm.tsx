...
export const UserProfileForm = ({
  ...
  const onSubmit = async (data: EditProfileSchema) => {
    try {
      ...
      if (errText.includes('already registered') || errText.includes('already exists')) {
        ...
        errText.includes('duplicate key value violates unique constraint "profiles_username_key"')
      ) {
        setError('username', {
          type: 'server',
        ...
        setError('root', {
          type: 'server',
          message:
            errorMessage || 'An error occurred while saving your profile. Please try again later.',
        });
      }
    }
  };
  ...
  const emailDescription =
    !errors.email?.message && isGoogleUser
