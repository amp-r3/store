...
const CHANGE_ROWS: ChangeRow[] = [
  ...
  {
    label: 'Category',
    read: (v, categories) => categories.find((c) => c.id === v.categoryId)?.name ?? null,
  },
  ...
  { label: 'Return policy', read: (v) => blankToNull(v.returnPolicy ?? '') },
  {
    ...
    read: (v) =>
      AVAILABILITY_STATUS_OPTIONS.find((o) => o.value === v.availabilityStatus)?.label ??
...
const buildMediaChanges = (
  ...
  const addSlot = (
    label: string,
    slot: { file: File | null; removed: boolean; existingUrl: string | null },
  ) => {
    if (slot.file) {
...
export const hasProductChanges = (changes: ProductChanges): boolean =>
