...
import { buildProductImageName, buildProductImageUrl, MAX_PRODUCT_IMAGES, ProductImageKind } from '@/shared/config';
import { AdminProductDetail, useUploadProductImageMutation, useRemoveProductImageMutation } from '@/entities/admin';
...
const buildSlot = (productId: number, kind: ProductImageKind, index: number | undefined, existingUrl: string | null): ProductMediaSlot => ({
...
export const useProductMediaDraft = (product?: AdminProductDetail): UseProductMediaDraftResult => {
    ...
    const selectThumbnailFile = useCallback((file: File) => {
        setThumbnail((current) => {
            ...
            validateProductImageFile(file, 'thumbnail').then(({ error, warning }) => {
                setThumbnail((latest) => (latest?.file === file ? { ...latest, isValidating: false, error, warning } : latest));
            });

            return { ...current, file, previewUrl, removed: false, isValidating: true, error: undefined, warning: undefined };
        });
    }, []);
    ...
    const removeThumbnail = useCallback(() => {
        setThumbnail((current) => {
            ...
            if (current.file) {
                ...
                return { ...current, file: null, previewUrl: current.existingUrl, error: undefined, warning: undefined };
            }
        ...
        (index: number, file: File) => {
            ...
            setGallery((current) => {
                ...
                validateProductImageFile(file, 'gallery').then(({ error, warning }) => {
                    setGallery((latest) =>
                        latest.map((s) => (s.index === index ? { ...s, isValidating: false, error, warning } : s))
                    );
                });

                return existingSlot ? current.map((s) => (s.index === index ? stagedSlot : s)) : [...current, stagedSlot];
            });
        },
        [productId]
    );
        ...
        setGallery((current) => {
            ...
            if (!slot.existingUrl) {
                return current
                    ...
                    .map((s, i) => ({ ...s, index: i + 1, fileName: buildProductImageName(productId!, 'gallery', i + 1) }));
            }

            if (slot.file) {
                return current.map((s) => (s.index === index ? { ...s, file: null, previewUrl: s.existingUrl, error: undefined, warning: undefined } : s));
            }

            return current.map((s) => (s.index === index ? { ...s, removed: true, previewUrl: null } : s));
        });
    }, [productId]);
    ...
    const commit = useCallback(async (): Promise<ProductMediaCommitResult> => {
        ...
        if (invalidSlot) {
            throw new Error(`Fix ${invalidSlot.kind === 'thumbnail' ? 'the thumbnail' : `image ${invalidSlot.index}`} before saving.`);
        }
