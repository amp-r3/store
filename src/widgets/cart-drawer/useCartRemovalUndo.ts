...
export const useCartRemovalUndo = ({ cartItems, cartDetails, isOpen }: UseCartRemovalUndoParams): UseCartRemovalUndoReturn => {
    ...
    const listRef = useRef<{ cartItems: CartProduct[]; cartDetails: (CartItemDetails | null)[] }>({ cartItems, cartDetails });
    useEffect(() => {
    ...
    const handleRemoved = useCallback((sizeId: number, productId: number, quantity: number) => {
    ...
    const handleCleared = useCallback((snapshot: Record<number, CartData>) => {
    ...
    return { removedEntries, clearedEntry, handleRemoved, handleCleared, dismissRemoval, dismissCleared };
};
