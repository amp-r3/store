...
export const MobileBar = () => {
  ...
  const activeLayer: DockLayer = isSearchOpen
    ? 'search'
    : productId && !isPurchaseBoxOnScreen && !isNavPinned
      ? 'purchase'
      : 'nav';
  ...
  const itemInCart = displayProduct
    ? cartItems.find(
        (item) => item?.productId === displayProduct.id && item?.sizeId === selectedSizeId,
      )
    : undefined;
  ...
  const { isSizeSelected, currentInStock, isMaxReached } = getPurchaseState({
    quantity,
    sizes: displaySizes,
    selectedSizeId,
    hasSizes,
