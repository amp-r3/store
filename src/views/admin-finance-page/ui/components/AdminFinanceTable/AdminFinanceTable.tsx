...
export const AdminFinanceTable = ({ days }: AdminFinanceTableProps) => {
  const {
    ...
    error: seriesError,
  } = useGetAdminFinanceSeriesQuery(days);
  const {
    data: summary,
    isLoading: isSummaryLoading,
    error: summaryError,
  } = useGetAdminFinanceSummaryQuery(days);
  const {
    data: breakdown,
    isLoading: isBreakdownLoading,
    error: breakdownError,
  } = useGetAdminFinanceBreakdownQuery(days);
    ...
    <div className={style['admin-finance-table']}>
      ...
      <section aria-label="Daily breakdown">
        <div className={style['admin-finance-table__grid']} role="table">
          ...
          {!isLoading && summary && (
            <div
              className={`${style['admin-finance-table__row']} ${style['admin-finance-table__row--total']}`}
              role="row"
            >
