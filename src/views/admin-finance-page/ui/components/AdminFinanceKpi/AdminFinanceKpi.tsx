    ...
    return (
        <div className={style['admin-finance-kpi']}>
            {isLoading || !summary ? (
                ...
                <>
                    <StatTile icon={<LuWallet />} label="Gross collected" value={formatPrice(summary.grossCollected)} />
                    <StatTile icon={<LuPackage />} label="Net to store" value={formatPrice(summary.itemsSubtotal)} />
                    <StatTile icon={<LuLandmark />} label="Payment fees" value={formatPrice(summary.paymentFees)} />
                    <StatTile icon={<LuUndo2 />} label="Refunded" value={formatPrice(summary.refundedAmount)} />
                </>
            )}
        </div>
    );
