...
export const AdminLowStockPanel = () => {
    const { data, isLoading, error } = useGetAdminLowStockQuery({ threshold: LOW_STOCK_THRESHOLD, limit: LOW_STOCK_LIMIT });
    const items = data ?? [];
    ...
    return (
        <PanelCard title="Low stock" to="/admin/products/low-stock">
            ...
            {isLoading ? (
                ...
                <EmptyState icon={<LuPackageOpen />} title="Stock levels look healthy" text="No size is at or below 5 units right now." />
            ) : (
                <div className={style['admin-low-stock-panel__list']}>
                    {items.map((item) => (
                        <Link
