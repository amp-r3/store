    ...
    <FilterPanel
      ...
      search={
        <form className={style['admin-products-toolbar__search']} onSubmit={handleSubmit}>
          <IoSearchOutline
            className={style['admin-products-toolbar__search-icon']}
            aria-hidden="true"
          />
          <input
            type="search"
      ...
      actions={
        <Link
          ...
          className={style['admin-products-toolbar__low-stock-link']}
        >
          <LuTriangleAlert /> Low stock
        </Link>
      }
    >
      <Switch label="Show archived" checked={includeArchived} onChange={onIncludeArchivedChange} />
    </FilterPanel>
  );
};
