...
export const AdminSettingsPage = () => {
  ...
  return (
    <>
      <SectionHeader title="Settings" subtitle="Delivery and payment methods used at checkout." />
      ...
      <section className={style['admin-settings-page__section']}>
        <div className={style['admin-settings-page__section-header']}>
          ...
          <p className={style['admin-settings-page__section-note']}>
            The set of methods is fixed — new ones can&apos;t be added and existing ones can&apos;t
            be removed, only edited or turned off.
          </p>
        </div>

        {!!deliveryQuery.error && (
          <Alert variant="error">{getErrorMessage(deliveryQuery.error)}</Alert>
        )}
      ...
      <section className={style['admin-settings-page__section']}>
        <div className={style['admin-settings-page__section-header']}>
          ...
          <p className={style['admin-settings-page__section-note']}>
            Same rule here — the method set is fixed by the database, only fees, name and activity
            change.
          </p>
        </div>

        {!!paymentQuery.error && (
          <Alert variant="error">{getErrorMessage(paymentQuery.error)}</Alert>
        )}
