...
export const AdminCategoryFormModal = ({ isOpen, onOpenChange, category }: AdminCategoryFormModalProps) => {
    ...
    const onSubmit = async (values: CategorySchema) => {
        setRootError(null);
        try {
            await upsertCategory({ id: category?.id ?? null, name: values.name, slug: values.slug }).unwrap();
            onOpenChange(false);
        } catch (err) {
        ...
        >
            <form className={style.form} onSubmit={handleSubmit(onSubmit)} noValidate>
                ...
                <FormField
                    ...
                    warning={slugChanged ? 'Changing the slug breaks existing links that use the old one.' : undefined}
